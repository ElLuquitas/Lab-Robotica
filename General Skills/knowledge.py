...
from uchile_skills.robot_skill import Log

from uchile_msgs.msg import SemanticObject as ps_msg
from uchile_srvs.srv import SemMap as ps_srv
from uchile_srvs.srv import String as string_srv
...
class PoseServerWrapper(object):
    ...
    Loads the default map on start-up (uchile_maps/maps/map.sem_map)
    Dumps to a backup map on shutdown (uchile_maps/maps/<current_map>.bkp)
    ...
    def setup(self):
        """
        The map is loaded by default with the map uchile_maps/maps/map.sem_map
        """
        return True
    ...
    def check(self):
        ...
        try:
            ...
            rospy.logwarn('[knowledge.pose]: Check failed. PoseServer services not found. e.g. /maqui/knowledge/pose_server/which')
            return False
        return True
